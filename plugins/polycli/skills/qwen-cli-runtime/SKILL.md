...
Primary helper:
- `node "${CLAUDE_PLUGIN_ROOT}/scripts/polycli-companion.bundle.mjs" task --provider qwen "<raw arguments>"`
...
- Do not call `setup`, `review`, `adversarial-review`, `status`, `result`, or `cancel` from `polycli:polycli-provider-agent`.
