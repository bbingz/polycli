...
Copy the smallest recipe that fits, then trim anything you don't need.
...
Gemini-specific: leverage the 1M token context window.

```xml
<task>
Analyze the provided large file(s) for patterns, issues, or the requested information.
You have a 1M token context window — use it to process the full content without chunking.
</task>
