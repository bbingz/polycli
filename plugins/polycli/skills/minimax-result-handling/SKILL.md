...
1. Spawned `mmx text chat --message <prompt> --output json --non-interactive`
2. Parsed the JSON response directly
3. Classified empty output, auth failures, and CLI failures into the shared Polycli result shape
...
3. **Never auto-execute.** MiniMax may suggest commands, code changes, or file edits. Do NOT apply them silently. Ask which items to act on.
...
The companion maps MiniMax outcomes to a fixed status set. Here is how Claude should open the message for each:
...
| `not-installed` | "mmx-cli is not installed. Run `/polycli:setup --provider minimax`." |
| `llm-call-failed` | "The MiniMax LLM call failed. Check mmx auth/region/model configuration." |
| `unknown-crashed` | "mmx-cli crashed unexpectedly. Check stderr and rerun." |
...
Token-usage story remains unsupported unless `mmx-cli` exposes stable token fields; do not claim costs.
