...
- **Near-empty bare-shell run flagged**: `qwen` `review` run #2 returned 1 byte (`"\n"`) at `exitCode: 0`. The bench treats it as success because the CLI exited 0. Min/max range for that cell shows `1-1879`; median (1556) is unaffected. Worth noting when reading raw data: 1/18 cells produced a near-empty success-coded response.
